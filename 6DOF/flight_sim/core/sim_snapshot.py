...
class SimSnapshot:
    def __init__(self, fs, sc: SimConditions, rocket: Rocket):
        self._state  = fs
        self._sc     = sc
        self._rocket = rocket
    ...
    @cached_property
    def altitude(self): return float(self._state.position.vector_world[2])
    @cached_property
    def pos_x(self): return float(self._state.position.vector_world[0])
    @cached_property
    def pos_y(self): return float(self._state.position.vector_world[1])

    # --- Velocity ---
    @cached_property
    def vel_x(self): return float(self._state.velocity.vector_world[0])
    @cached_property
    def vel_y(self): return float(self._state.velocity.vector_world[1])
    @cached_property
    def vel_z(self): return float(self._state.velocity.vector_world[2])
    @cached_property
    def speed(self): return float(np.linalg.norm(self._state.velocity.vector_world))
