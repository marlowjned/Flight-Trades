# SimulationConditions.py
# Computed once per timestep from FlightState + WindModel.
...
class SimConditions:
    ...
    def compute(
        ...
        u, v, w = wind.windVector(state.position.z).vectorWorld
        wind_world = np.array([u, v, w])
        airflow = Vector3D(wind_world - state.velocity.vectorWorld,
